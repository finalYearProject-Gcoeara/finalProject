...
function delay(ms) {
  ...
  function extractData(html) {
    ...
    // Extract reviews from review containers
    const reviewsData = [];
    ...
    // Look for a "Next" button to get subsequent review pages (adjust selector if needed)
  ...
  async function scrapeAllPages(startUrl) {
    ...
    // Loop through pages as long as a next page exists.
    while (currentPage) {
      pageCount++;
      ...
      try {
        ...
        if (productData.next_page) {
          // Resolve next page URL relative to the current page
          currentPage = new URL(productData.next_page, currentPage).href;
          // Wait 2 seconds before fetching the next page
          await delay(2000);
        } else {
          currentPage = null;
        }
  ...
  async function main() {
    // Use current page URL as starting point

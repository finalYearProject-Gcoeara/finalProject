...
//               await delay(2000); // Adding delay between requests
//           } else {
//               currentPage = null;
//           }
//       } catch (error) {
...
async function fetchHTML(url) {
  ...
  async function main() {
    ...
    // Extract the product name from the first product's data (if available)
    const productName = allPagesData.length > 0 ? allPagesData[0].product_name : "Unknown Product";
    
    // Display an alert once scraping is complete with product name and total pages
    alert(`Scraping completed for "${productName}`);
